...
int box_dossier::dossi_boxifil(int number,float rst,int rowsx,int columnsz,string snumber,int preci){
   ...
   for(int j=0;j<rowsx;j++){
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(preci));
        ar_main[pos_idx]=per_line_isia;
        ar_secon[pos_idx]=per_line_plagia;
        pos_idx+=1;
...
cout<<"counter of result table ::== " << pin_ctl <<"\n";
cout<<"\n";
cout<<"---oftype of main----\n";
...
int box_dossier::show_results(){
  cout<<"Isia : \n";
  cout<<"@i     value_of   sum_of\n";
  for(int i=0;i<=pin_ctl;i++){
    cout<< i+1  << "        " << isios_sum[i] << "        " <<  isios_subsum[i] <<"\n";
  }
  cout<<"\n";
  cout<<"Plagia : \n";
  cout<<"@i     value_of   sum_of\n";
   for(int i=0;i<=pin_ctl;i++){
    cout<<i+1 <<"        " << plagios_sum[i] << "        " << plagios_subsum[i] <<"\n";
  }
  cout<<"\n";
  return 0;
}
